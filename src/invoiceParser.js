...
function normalizeNumber(raw) {
  ...
  const withDots = cleaned.replace(/\./g, '').replace(/,/g, '.');
...
function isInvoiceMarker(text) {
  return /factura\s*(n|no|nº|n°|#)/i.test(text);
}

function pickSupplier(lines) {
  const labeled = lines.find((line) => /proveedor|raz[oó]n\s+social|emisor|empresa|vendor|supplier/i.test(line));
  if (labeled) {
    const [, value] = labeled.split(/:\s*/);
    if (value) return value.trim();
  }

  const cuitIndex = lines.findIndex((line) => /cuit|cuil/i.test(line));
  if (cuitIndex > 0) {
    const candidate = lines[cuitIndex - 1]?.trim();
    if (candidate && !isDateLike(candidate) && !isInvoiceMarker(candidate)) return candidate;
  }

  const fallback = lines.find((line) => {
    if (!line || line.trim().length <= 3) return false;
    if (isDateLike(line) || isInvoiceMarker(line)) return false;
    const hasLetters = /[a-zA-Z]/.test(line);
    const hasDigits = /\d/.test(line);
    return hasLetters && line.length > 4 && (!hasDigits || /srl|sa|sas|ltda|corp|inc/i.test(line));
  });

  const firstPlainCandidate = lines.find((line) => {
    if (!line || line.trim().length <= 3) return false;
    if (isDateLike(line) || isInvoiceMarker(line)) return false;
    return /[a-zA-Z]/.test(line);
  });

  return fallback || firstPlainCandidate || lines.find((line) => line.trim().length > 3) || '';
}
...
function pickItems(lines) {
  ...
  const endIndex = lines.findIndex((line) => /total\s*(bruto|general|neto)?|subtotal/i.test(line));
  const windowed = endIndex > start ? lines.slice(start, endIndex) : lines.slice(start);

  const candidates = windowed.filter((line) => {
    if (!line || line.trim().length < 4) return false;
    if (/^cant\b|^codigo\b|^articulo\b|^precio\b/i.test(line)) return false;
    if (isDateLike(line)) return false;
    const numericCount = (line.match(/[$€¥£]?[-+]?\d[\d.,]*/g) || []).filter(Boolean).length;
    const hasLetters = /[a-zA-Z]/.test(line);
    return hasLetters && numericCount >= 2;
  });
